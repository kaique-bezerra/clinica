...
function Agendamento() {
  ...
  // Estados para Pacientes e Autocomplete
  ...
  // --- CARREGAR MÉDICOS E PACIENTES DO BACKEND (GET) ---
  useEffect(() => {
    async function carregarDados() {
      ...
      // Buscar Médicos
      try {
        const resMedicos = await fetch("http://localhost:8080/medico", { method: "GET", headers });
        if (resMedicos.ok) {
      ...
      // Buscar Pacientes
      try {
        const resPacientes = await fetch("http://localhost:8080/paciente", { method: "GET", headers });
        if (resPacientes.ok) {
  ...
  function handleBuscaPacienteChange(event: React.ChangeEvent<HTMLInputElement>) {
    ...
    const filtrados = pacientes.filter(paciente => {
      ...
      const cpfPacienteNumeros = paciente.cpf ? paciente.cpf.replace(/\D/g, "") : "";

      const combinaNome = nomeCompleto.includes(valorMinusculo);
      const combinaCpf = termoApenasNumeros !== "" && cpfPacienteNumeros.includes(termoApenasNumeros);
  ...
  function selecionarPaciente(paciente: Paciente) {
    setTermoBuscaPaciente(`${paciente.nome} ${paciente.sobrenome || ""} - CPF: ${paciente.cpf || "Não informado"}`);
    setFormData(prev => ({ ...prev, idPaciente: String(paciente.id) }));
    setMostrarSugestoes(false);
  }

  // --- HANDLER DE ALTERAÇÃO DOS OUTROS INPUTS ---
  ...
  async function handleSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    if (!formData.idPaciente) {
      alert("Por favor, selecione um paciente válido na lista de sugestões.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          alert("Erro 403: Seu usuário não tem permissão para agendar consultas.");
          return;
        }
        const textError = await response.text();
        alert(`Erro ao agendar consulta: ${textError}`);
        return;
      }
          ...
          <form className="patient-form" onSubmit={handleSubmit} autoComplete="off">
            ...
            <fieldset className="form-group-box">
              ...
              <div className="form-row">
                {/* CAMPO DE BUSCA COM TABELA FLUTUANTE (MANTIDO 100% ORIGINAL) */}
                <div className="input-group" style={{ position: "relative" }}>
                  ...
                  <input 
                    ...
                    onFocus={() => termoBuscaPaciente && setMostrarSugestoes(true)}
                  ...
                  {mostrarSugestoes && termoBuscaPaciente && sugestoesFiltradas.length === 0 && (
