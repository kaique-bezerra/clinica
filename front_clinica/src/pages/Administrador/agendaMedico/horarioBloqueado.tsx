    ...
    try {
      ...
      if (!response.ok) {
        if (response.status === 403) {
          setMensagem(
            "Erro 403: Seu usuário não tem permissão para cadastrar bloqueios.",
          );
          return;
        }
        const textError = await response.text();
        setMensagem(`Erro ao cadastrar o bloqueio: ${textError}`);
        return;
      }

      setMensagem("Horário bloqueado cadastrado com sucesso! 🔒");
    ...
    <div className="pacientes-container">
      ...
      <main className={`main-content ${menuAberto ? "expanded" : ""}`}>
        <section className="page-header">
          ...
          <p>
            Impeça novos agendamentos definindo ausências, reuniões ou férias
            médicas.
          </p>
        </section>
